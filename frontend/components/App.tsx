...
import SplitDmTab from './tabs/DmTab';
import SplitChannelTab from './tabs/ChannelTab';
import SplitMyPageTab from './tabs/MyPageTab';
import SplitSurveyTab from './tabs/SurveyTab';
import SplitCollectTab from './tabs/CollectTab';
            ...
            return (
                <div className="flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-hidden relative">
                        {tab === 'dm' && <SplitDmTab user={user} authUrl={authUrl} runGas={runGas} fetchAuthUrl={fetchAuthUrl} processInBatches={processInBatches} DialogModal={DialogModal} />}
                        {tab === 'channel' && <SplitChannelTab user={user} authUrl={authUrl} runGas={runGas} fetchAuthUrl={fetchAuthUrl} processInBatches={processInBatches} DialogModal={DialogModal} />}
                        {tab === 'mypage' && <SplitMyPageTab runGas={runGas} DialogModal={DialogModal} />}
                        {tab === 'roster' && <RosterTab user={user} />}
                        {tab === 'survey' && <SplitSurveyTab user={user} runGas={runGas} DetailsModal={DetailsModal} />}
                        {tab === 'collections' && <SplitCollectTab user={user} runGas={runGas} parseCsv={parseCsv} formatPreviewBirthday={formatPreviewBirthday} DetailsModal={DetailsModal} DialogModal={DialogModal} />}
                    </div>
                </div>
            );
        }

        function DetailsModal({ isOpen, loading, details, onClose, survey }) {
            if (!isOpen) return null;
            return (
